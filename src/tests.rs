...
mod tests {
    ...
    fn temp_arena_rewinds_allocations() {
        ...
        {
            let temp = arena.temp();
            let mut vec = ArenaVec::with_capacity_in(1, &temp);
            vec.push([1_u32, 2, 3, 4]);
            vec.push([5_u32, 6, 7, 8]);
            assert!(temp.used() > used_before);
        }
    ...
    fn arena_vec_grows_and_preserves_contents() {
        ...
        let mut vec = ArenaVec::with_capacity_in(1, &scope);
    ...
    fn arena_vec_rejects_droppable_types() {
        ...
        let result = catch_unwind(AssertUnwindSafe(|| {
            let scope = arena.scope();
            let mut vec = ArenaVec::new_in(&scope);
    ...
    fn string8_keeps_trailing_nul_and_excludes_it_from_length() {
        let mut arena = Arena::new(1024);
        let scope = arena.scope();
        let string = String8::from_str_in("gin", &scope);
    ...
    fn string8_append_and_clear_work() {
        let mut arena = Arena::new(1024);
        let scope = arena.scope();
        let mut string = String8::new_in(&scope);
